	...
	)
	{
		foreach($arResult["POST"] as $ind => $CurPost)
		{
			...
			?>
				...
				<article class="bs-<?=$className?>" id="blg-post-<?=$CurPost["ID"]?>">
					<h2 class="bs-blog-post-title"><a href="<?=$CurPost["urlToPost"]?>" title="<?=$CurPost["TITLE"]?>"><?=$CurPost["TITLE"]?></a></h2>
					<div class="bs-blog-post-meta">
						<!-- rating -->
					...
					<div class="blog-post-meta">
						...
						<div class="blog-post-meta-util">
							<span class="blog-post-views-link"><a class="link-dark" href="<?=$CurPost["urlToPost"]?>"><span class="blog-post-link-caption"><?=GetMessage("BLOG_BLOG_BLOG_VIEWS")?></span><span class="blog-post-link-counter"><?=intval($CurPost["VIEWS"]);?></span></a></span>
							...
							<?if($CurPost["urlToHide"] <> ''):?>
								<span class="blog-post-hide-link"><a class="link-secondary" href="javascript:if(confirm('<?=GetMessage("BLOG_MES_HIDE_POST_CONFIRM")?>')) window.location='<?=$CurPost["urlToHide"]."&".bitrix_sessid_get()?>'"><span class="blog-post-link-caption"><?=GetMessage("BLOG_MES_HIDE")?></span></a></span>
							<?endif;?>
							<?if($CurPost["urlToEdit"] <> ''):?>
								<span class="blog-post-edit-link"><a class="link-warning" href="<?=$CurPost["urlToEdit"]?>"><span class="blog-post-link-caption"><?=GetMessage("BLOG_MES_EDIT")?></span></a></span>
							<?endif;?>
							<?if($CurPost["urlToDelete"] <> ''):?>
								<span class="blog-post-delete-link"><a class="link-danger" href="javascript:if(confirm('<?=GetMessage("BLOG_MES_DELETE_POST_CONFIRM")?>')) window.location='<?=$CurPost["urlToDelete"]."&".bitrix_sessid_get()?>'"><span class="blog-post-link-caption"><?=GetMessage("BLOG_MES_DELETE")?></span></a></span>
							<?endif;?>
